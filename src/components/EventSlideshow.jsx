...
const EventSlideshow = () => {
  ...
  return (
    <div className="w-full border-t-4 border-b-4 border-black py-8 my-16">
      {/* Section Header */}
      <div className="flex items-end justify-between mb-12 px-6 lg:px-24">
        <div>
          <p className="text-sm font-black uppercase tracking-[0.3em] text-gray-400 mb-2">Featured Events</p>
          <h2 className="text-5xl md:text-7xl font-black uppercase text-black tracking-tighter leading-none">
        ...
        <div className="flex gap-4 pb-2">
          <button
            onClick={prev}
            className="w-14 h-14 border-4 border-black bg-white hover:bg-black hover:text-white flex items-center justify-center transition-all duration-200 group shadow-[4px_4px_0px_rgba(0,0,0,1)] hover:shadow-none active:translate-x-1 active:translate-y-1"
          >
            <ArrowLeft className="w-6 h-6" />
          </button>
          <button
            onClick={next}
            className="w-14 h-14 border-4 border-black bg-black text-white hover:bg-blue-600 hover:border-blue-600 flex items-center justify-center transition-all duration-200 shadow-[4px_4px_0px_rgba(0,0,0,1)] hover:shadow-none active:translate-x-1 active:translate-y-1"
          >
      ...
      <div className="px-6 lg:px-24">
        <AnimatePresence mode="wait" custom={direction}>
          <motion.div
            ...
            className="grid grid-cols-1 lg:grid-cols-2 gap-0 border-4 border-black shadow-[8px_8px_0px_rgba(0,0,0,1)]"
          >
            {/* Image Side */}
            ...
            {/* Description Side */}
            <div className="flex flex-col justify-between p-10 lg:p-16 bg-white">
              {/* Slide counter */}
                ...
                <div className="flex gap-2">
                  {events.map((_, idx) => (
                    <button
                      ...
                      className={`h-2 transition-all duration-500 border-2 border-black ${idx === current ? 'w-10 bg-black' : 'w-4 bg-transparent'}`}
                    />
                  ))}
                </div>
                <span className="font-black text-gray-300 text-4xl">{String(current + 1).padStart(2, '0')}</span>
              </div>
              ...
              <div className="flex-1 flex flex-col justify-center">
                ...
                <h3 className="text-5xl lg:text-6xl font-black uppercase text-black leading-none tracking-tighter mb-8">
                  {event.title}
                </h3>
